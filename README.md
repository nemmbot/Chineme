...
- Search provider benchmark: Run bots using different combinations of search providers (e.g. Google, Bing, Exa.ai, Tavily, AskNews, Perplexity, OpenRouter, Sonar Pro, GPT-5, etc) and search filters (e.g. only recent data, sites with a certain search rank, etc) and see if any specific one is better than others, or if using multiple of them makes a difference.
- The current tournament pipeline now includes AI tournament ID `33022`, `minibench`, `market-pulse-26q1`, and `market-pulse-26q2`, with extreme extremization enabled on forecast outputs.
